...
runners.forEach((runners) =>{
    let firstLast = runners.first_name + " " + runners.last_name;
    fullName.push(firstLast);
});
...
let ticketPriceTotal = runners.reduce((sum, runners)=> sum + runners.donation,0);
...
// Problem 1
//The director wants to honor those who donated more than $200 so he needs the information of those people

let generousDonor = runners.filter((runners)=>{
    return runners.donation > 200; 
  });
  console.log(generousDonor);


// Problem 2
//The director needs to know how many small shirts to buy
let c = 0;
runners.forEach((runners) =>{
...
// Problem 3
//The director wants to split up the runners into two teams for a team competition after the race. Provide an array of all runners with id less than 25
let firstTeam = runners.filter((runners)=>{
